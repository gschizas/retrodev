...
def extrae_paleta(fichero_imagen, modo_imagen):
    ...
    if (modo_imagen == "P"):
        ...
        for i in range(cantidad_de_colores):
            ...
            if (paleta_aux in paleta_cpc):
                paleta_final[paleta_aux] = (i, paleta_cpc[paleta_aux])
            else:
...
def extrae_imagen(fichero_imagen, modo_imagen, ancho_imagen, alto_imagen, paleta):
    ...
    if (modo_imagen=="RGB") or (modo_imagen=="RGBA"):
        for i in range(alto_imagen * ancho_imagen):
            imagen_final[i] = paleta[chr(imagen_final[i][0]) + chr(imagen_final[i][1]) + chr(imagen_final[i][2])][0]
    elif (modo_imagen != "P"):
        imagen_final = []
        print u"Modo no soportado."
    imagen_final = map(chr, imagen_final)
    return imagen_final
...
def main(linea_de_comandos=None):
    ...
    for nombre_imagen in lista_ficheros:
        ...
        if not opciones.sprite:
            ...
            imagen = extrae_imagen(fichero_imagen, modo_imagen, ancho_imagen, alto_imagen, paleta)
